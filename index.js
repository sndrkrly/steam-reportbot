...
(async() => {
	...
	if (config.limited == true) 
		console.log('You set LIMITED accounts. Use this format in the BOTS.TXT:'.black.bgWhite + "login:password:".blue.bgWhite+'(with the colon in the end of lines)'.black.bgWhite);
		...
		async.each(subbot[ii], function(item, callback){
								...
								(async() => {					
									...
									var options = {
										formData: {	
											...
											abuseID: config.id, 
											eAbuseType: 10, 
											abuseDescription: 'Cheating in the recent matches.', 
											ingameAppID: config.appid 
										},
									...
									community.httpRequestPost('https://steamcommunity.com/actions/ReportAbuse/', options, function (err, res, data) {
										if (err) {
											console.log('err', err); 
											failed++; allFailed++;
										}
										...
										if (!err) {
										 	if (data == 1) { 
												console.log('[%s] Successfully reported with response code %s'.green, logOnOptions.accountName, data); 
												success++; allSuccess++;
											} else if (data == 25) { 
												console.log('[%s] Already reported. Response code %s'.red, logOnOptions.accountName, data); 
												failed++; allFailed++; 
											} else { 
												console.log('[%s] something went wrong. Response code %s'.red, logOnOptions.accountName, data); 
												failed++; allFailed++;
											}
				...
				if (limited) {
					...
					community.login({
						...
						function (err, sessionID, cookies, steamguard, oAuthToken) {
							if (err) { 
								console.log('[%s] Unable to auth (Error: %s)'.red, logOnOptions.accountName, err); 
								failed++; allFailed++; callback(); 
							}
				...
				if (ii < subbot.length - 1) 
					console.log('Waiting %s ms for the next chunk'.cyan, betweenChunks);
		});
